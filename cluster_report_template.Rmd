...
```
The analysis of your samples has been completed. The sample identifiers included on this analysis should be treated as patient data for privacy purposes. Additionally, the results of this analysis are to be used solely for surveillance purposes and should not be used to inform patient care. These genomic data must always be used in conjunction with epidemiological data when determining if isolates are epidemiologically linked. This analysis should not be used as a replacement for a thorough epidemiological investigation.
...
consensus_metrics <- status_table
consensus_metrics <- cbind(rownames(consensus_metrics), data.frame(consensus_metrics, row.names=NULL))
colnames(consensus_metrics) <- c("Samplename", "Status")
...
This workflow is based upon the cluster analysis pipeline from the [Monroe workflow](https://github.com/StaPH-B/staphb_toolkit/tree/dev).

This workflow uses [Mafft](https://mafft.cbrc.jp/alignment/software/) to perform multiple-sequence alignment of all genomes provided. The resulting alignment fasta file is used to generate a pairwise-snp distance matrix with [snp-dists](https://github.com/tseemann/snp-dists) and a maximum-likelihood phylogeneitc tree with [IQ-Tree](http://www.iqtree.org/). Output from snp-dists and IQ-Tree are curated into a single pdf report.
