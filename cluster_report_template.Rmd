...
\fancyhead[R]{\Huge MASPHL Genomic Clustering Report\\
...
```
The analysis of your samples has been completed. The sample identifiers included on this report should be treated as patient data for privacy purposes. Additionally, the results of this report are to be used solely for surveillance purposes and should not be used to inform patient care. These results must always be used in conjunction with epidemiological data when determining if isolates are epidemiologically linked. This analysis should not be used as a replacement for a thorough epidemiological investigation.

## Sample Sequence Pass/Fail Status
```{r, table1, echo = FALSE}

#metrics <- read.table(status_table, header = TRUE, sep = '\t')

# Separating out metrics file assembly info
columns <- c("coverage_after_trimming", "meanbaseq_trim", "meanmapq_trim", "assembly_status")
consensus_metrics <- status_table[, columns]
colnames(consensus_metrics) <- c("Coverage", "Avg. Base Q", "Avg. Map Q", "Status")
...
Monroe cluster_analysis uses [Mafft](https://mafft.cbrc.jp/alignment/software/) to perform multiple-sequence alignment of all genomes provided. The resulting alignment fasta file is used to generate a pairwise-snp distance matrix with [snp-dists](https://github.com/tseemann/snp-dists) and a maximum-likelihood phylogeneitc tree with [IQ-Tree](http://www.iqtree.org/).

Output from snp-dists and IQ-Tree are curated into a single pdf report using a [MASPHL version](https://github.com/AndrewLangvt/genomic_report) of the [StaPH-B cluster-report-env](https://hub.docker.com/r/staphb/cluster-report-env).
